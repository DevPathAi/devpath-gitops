# CLAUDE.md — devpath-gitops

> Kubernetes 매니페스트 + ArgoCD 배포 설정 (GitOps)
...
산출물이 코드가 아니라 K8s 매니페스트다. 절대 조건 2의 "테스트 우선"은 **변경을 적용하기 전 매니페스트를 렌더·검증**하는 것으로 적용한다.

- 렌더 검증: `kubectl kustomize apps/<svc>/base`
- 적용 전 dry-run: `kubectl apply --dry-run=server -k apps/<svc>/base`
- 새 서비스: `apps/<name>/base/`에 Kustomize 매니페스트 추가 → `argocd/applicationset.yaml`이 자동 발견.
- 구조/부트스트랩은 [README](README.md), 로컬 클러스터는 [local-k8s](local-k8s/README.md) 참고.
...
- 이 레포 관련 자산:
  ...
  - 가이드: Kubernetes 배포 전략 · Docker 설치·운영
