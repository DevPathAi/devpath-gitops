...
STEP_TARGET_CHECKOUT = "Checkout the exact tested pipeline-defects target"
STEP_TARGET_TEST = "Test the exact pipeline-defects target"
...
STEP_NAMES = [
    ...
    'Checkout the exact tested pipeline-defects target',
    None,
    ...
    'Fast-forward protected main to the exact tested pipeline-defects target',
...
GATE_BODIES = {
    'Prove the exact one-shot helper context': [
        ...
        'test "${helper_paths[1]}" = "tests/release/test_pipeline_defects_main_publish.py"',
        'current_main="$(',
    ...
    'Run the publisher contract test on the exact helper bytes': [
        ...
        "python -m unittest discover -s tests/release -p 'test_pipeline_defects_main_publish.py'",
    ],
    'Authenticate the live protected environment and this approval': [
        'set -euo pipefail',
        'environment_json="$(',
    ...
    'Test the exact pipeline-defects target': [
        ...
        'test "$(git show -s --format=%s HEAD)" = \'release: add the production startup budget, the fence pull secret, and the landing API smoke to main\'',
        ...
        'expected_listing="$(printf \'%s\\n\' $\'M\\tapps/devpath-ai-svc/base/deployment.yaml\' $\'M\\tapps/devpath-community-svc/base/deployment.yaml\' $\'M\\tapps/devpath-gateway/base/deployment.yaml\' $\'M\\tapps/devpath-lcs-svc/base/deployment.yaml\' $\'M\\tapps/devpath-learning-svc/base/deployment.yaml\' $\'M\\tapps/devpath-migration/base/writer-fence-rbac.yaml\' $\'M\\tapps/devpath-notification-svc/base/deployment.yaml\' $\'M\\tapps/devpath-platform-svc/base/deployment.yaml\' $\'M\\tapps/devpath-sandbox-svc/base/deployment.yaml\' $\'M\\tscripts/release/cloudflare_pages.py\' $\'M\\ttests/release/test_cloudflare_api.py\' $\'A\\ttests/release/test_production_startup_budget.py\')"',
...
class PipelineDefectsMainPublishTest(unittest.TestCase):
    @classmethod
    ...
    def test_job_is_the_only_job_and_is_fenced(self) -> None:
        ...
        self.assertEqual(
            {"group": "pipeline-defects-main-publish", "cancel-in-progress": False},
