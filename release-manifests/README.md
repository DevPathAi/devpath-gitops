...
- exact repository/source SHA, catalog path/version/SHA-256, case count, and render/test input-provenance SHA-256 for all seven quality lanes; frontend v1 separates the common `leva.et13.catalog.v1` contract version from each lane's generated-file schema version, prebinds both the canonical input-provenance digest (sorted-key compact JSON with the self-digest excluded) and the raw `provenance.v1.json` file digest, fixes the ordered 12-fixture ID list, the generated visual catalog at 96 cases (`Web` 48, `Admin` 16, `Mobile` 16, `dp_design` 16), and the generated automated-accessibility catalog at 24 cases (`Web` 12, `Admin` 4, `Mobile` 4, `dp_design` 4); the visual lane additionally prebinds an approved baseline set and approval digest; both frontend catalogs require `evidence_mode=release_ready`, `capture_surface=flutter_web_release_projection`, and `device_evidence=false`, so diagnostic/pending output is structurally unsealable and their `Mobile` cases cannot satisfy signed native-build or manual TalkBack evidence;
