...
- exact repository/source SHA, catalog path/SHA-256, case count, and render/test provenance SHA-256 for all seven quality lanes; the frontend visual and automated-accessibility counts must equal the sum of the exact `Web`, `Admin`, `Mobile`, and `dp_design` surface counts in their result, with no arbitrary upper bound on catalog size;
